# API Flexibility Analysis

A written reflection comparing REST and GraphQL API design philosophies, analyzing their respective trade-offs, and exploring when each approach is the better architectural choice.

## Resources

- [GraphQL vs REST: A Comprehensive Comparison — Hygraph](https://hygraph.com/blog/graphql-vs-rest-apis) *(Article)*
- [Is GraphQL Better Than REST? — Tailcall](https://tailcall.run/graphql/graphql-vs-rest-api-comparison/) *(Article — link unavailable at time of submission)*
- [GraphQL vs REST: API Design Patterns and Best Practices — Calmops](https://calmops.com/web/graphql-vs-rest-api-design/) *(Article — substituted for unavailable Tailcall resource)*
- [GraphQL vs REST: API Showdown — Medium](https://medium.com/@programordie/graphql-vs-rest-api-showdown-fcf11d9c4fe6) *(Article)*

## Reflection Questions

1. **The Over-fetching Problem** — In your own words, explain the concepts of "over-fetching" and "under-fetching" in the context of a REST API. How does GraphQL's query language inherently solve this problem?

2. **Endpoint and Schema Philosophy** — Compare the endpoint structure of a typical REST API with that of a GraphQL API. How does GraphQL's strongly-typed schema benefit frontend developers?

3. **Caching and Complexity** — Caching is often cited as a major advantage of REST. Explain why caching is simpler in REST compared to GraphQL. What is a use case where the flexibility of GraphQL might outweigh its caching complexity?

## Submission

- [reflections.md](./reflections.md)